...
### Стоимость предметом: `sdm:cost_item`

Списывает конкретный ItemStack из инвентаря игрока.
...
| Параметр | Тип | Обязательно | Описание |
|---|---:|:---:|---|
| `item` | resource location | Да | ID предмета, который списывается. |
| `amount` | int | Нет | Базовое количество предметов до promo-модификаторов. По умолчанию `1`. Итоговое списываемое число округляется вверх. |
| `nbt` | string | Нет | Опциональный SNBT, который должен совпадать у списываемого ItemStack. |
| `group_id` | string | Нет | Группа оплаты. Пустая строка — группа по умолчанию. |

### Стоимость тегом предметов: `sdm:cost_item_tag`

Списывает любые предметы, входящие в item tag.
...
| Параметр | Тип | Обязательно | Описание |
|---|---:|:---:|---|
| `tagKey` | resource location | Да | ID item tag без символа `#`. |
| `amount` | int | Нет | Базовое количество предметов до promo-модификаторов. По умолчанию `1`. Итоговое списываемое число округляется вверх. |
| `group_id` | string | Нет | Группа оплаты. Пустая строка — группа по умолчанию. |

Редактор даёт поисковый TagKey-выбор. Этот компонент использует item tags; block tags показываются только для полей типа `TagKey<Block>`.
...
### Акция по дням недели: `sdm:promo_weekly_time`

Акция активна в выбранные дни недели. Время внутри дня можно не указывать — тогда акция работает весь выбранный день.
...
Пример акции на весь понедельник:
...
| Параметр | Тип | Обязательно | Описание |
|---|---:|:---:|---|
| `days` | string[] | Да | Дни недели, когда акция может быть активна: `MONDAY`, `TUESDAY`, `WEDNESDAY`, `THURSDAY`, `FRIDAY`, `SATURDAY`, `SUNDAY`. Можно указать несколько дней. |
| `start_time` | string | Нет | Начало окна внутри дня. Можно писать час (`"6"`), время (`"06:00"`) или минуты от начала суток (`"360"`). Если `start_time` и `end_time` не указаны, акция работает весь день. |
| `end_time` | string | Нет | Конец окна внутри дня. Формат такой же, как у `start_time`. |
| `promo_id` | string | Нет | ID акции. |
| `scope` | string | Нет | Общее promo-поле. Обычно достаточно `GLOBAL`, если акция зависит только от календаря. |

Если `start_time` больше `end_time`, интервал считается переходящим через полночь. Например `22:00` → `02:00` активируется вечером выбранного дня и продолжится до ночи следующего дня.
...
| `icon_type` | string | Нет | Режим иконки: `NONE`, `ITEM` или `TEXTURE`. По умолчанию `NONE`. |
| `icon_item` | item stack SNBT | Нет | ItemStack-иконка, если `icon_type` равен `ITEM`. |
| `icon_texture` | resource location | Нет | Текстура, если `icon_type` равен `TEXTURE`. Пример: `modid:textures/gui/icon.png`. |

Примечания по иконкам:

- `icon_item` использует полный SNBT ItemStack, например `{id:"minecraft:diamond",Count:1b}`.
- В редакторе ЛКМ по слоту иконки категории открывает выбор предмета, а ПКМ — выбор текстуры. Окно выбора текстуры показывает загруженные текстуры в сетке с поиском.
